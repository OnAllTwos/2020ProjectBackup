//Yet another old evolution simulator, nothing really special about this one
...
void draw(){
  ...
  for(int i=0;i<35;i++){  //Draws food at position
    fill(0,255,8);
  ...
  fill(0,100,0);
  textSize(100);
  text(myEggs.length+" Eggs",100,100);
}
public void eatFood(){
  for(int i=0; i<35; i++){
...
public void genBugs(){
  for (int i=0; i<myBugs.length; i++){
    ...
    myBugs[i].energyconsumption = (abs(myBugs[i].speedx)+abs(myBugs[i].speedy))*(0.003+(abs(myBugs[i].speedx)+abs(myBugs[i].speedy))/1000+(myBugs[i].size*0.00001));
  }
}
...
public void checkReproduce(){
  for(int i=0; i<myBugs.length; i++){
    if(myBugs[i].energy > myBugs[i].reproductioncap && myBugs[i].reproductioncooldown<=0 && myBugs[i].living && myBugs[i].age > myBugs[i].maturityage){
      ...
      tempEgg[0].energyconsumptionA = (tempEgg[0].speedxA+tempEgg[0].speedyA)*(0.003+(tempEgg[0].speedxA+tempEgg[0].speedyA)/2000);
